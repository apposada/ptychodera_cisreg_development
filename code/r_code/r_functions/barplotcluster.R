...
barplot_cluster <- function(x, clu_method = "ward.D2", dist_method="euclidean", main_label = "", bar_colors = colorspace::hex(HSV(head(seq(0,360,length.out=ncol(x)+1),-1),.5,1))){ #add clumethod,color pal, ...
  
  # Clustering
  hc=hclust(dist(x, method = dist_method), method=clu_method)
  hc=reorder(
    hc,
  ...
  cowplot::plot_grid(prow,p2_legend,rel_widths = c(1, .3))
}

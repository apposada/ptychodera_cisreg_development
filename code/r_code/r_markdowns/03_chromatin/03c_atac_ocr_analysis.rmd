...
dist_bin <- 
  as.numeric(cut(
    abs(peaks_genes_less100kb$distance),
    breaks = seq(0, 100000, by = 100),
    labels = seq(100, 100000, by = 100),
    include.lowest = TRUE
    ))

dist_bin <-
  dist_bin * ifelse(peaks_genes_less100kb$distance <0,-100,100)

plot(
  ecdf(dist_bin),
  col="#323e9f",
  main = "Cumulative density of\nOCR distance to genes",
...
atac_by_rna_tiers$stage <- 
  factor(
    setNames(
      c("eb", "eg", "mg", "lg"),1:4),
      levels = c("eb", "eg", "mg", "lg")
    )[atac_by_rna_tiers$stage]

# whole dataset
agreement_rna_atac_whole <-
   ggplot(data = atac_by_rna_tiers) +
  geom_jitter(
    aes(x = stage[], y = counts[], color = tier),
    position = position_jitterdodge(), size = .75,
    alpha = .15) +
  geom_boxplot(aes(x = stage, y = counts, fill = tier), outlier.shape = NA) +
  scale_fill_manual(values = c(sequential_hcl(5, "YlGnBu")[4:2])) +
  scale_color_manual(values = darken(sequential_hcl(5, "YlGnBu")[4:2], .5)) +
  theme_minimal() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank())

# Sample 500 data points for each combination of stage and tier
set.seed(456)
agreement_subspl <- atac_by_rna_tiers %>%
  group_by(stage, tier) %>%
  sample_n(1000, replace = TRUE) %>%
  ungroup()

agreement_rna_atac <-
  ggplot(data = agreement_subspl) +
  geom_boxplot(aes(x = stage, y = counts, fill = tier), outlier.shape = NA) +
  geom_jitter_rast(
    aes(x = stage[], y = counts[], color = tier),
    position = position_jitterdodge(), size = .75,
...
lines(density(homer_identity$V4), col = "darkred", lwd = 2)
```
...
pfla_motifs_peakcategory$category <- 
  factor(
    pfla_motifs_peakcategory$category,
    levels = c("Intergenic", "promoter-TSS", "exon", "intron", "TTS")
  )
...
diffpeaks_assoc_allgenes <-
  allpeaks_assoc_allgenes[allpeaks_assoc_allgenes$id_peak %in% rownames(res_atac_signif),]
...
diff_atac_category_homer <-
  atac_category_homer[
    atac_category_homer$peakID %in% rownames(res_atac_signif),
  ]
...
```{r ALL PLOTS}
pdf("graphics/atac_ocrs_cumul_dens.pdf", width = 6, height = 6)
dist_bin <- 
  as.numeric(cut(
    abs(peaks_genes_less100kb$distance),
    breaks = seq(0, 100000, by = 100),
...
pdf("graphics/Figures_ATAC_OCRs.pdf", height = "8", width = "8")
# num peaks by genomic feature
par(mfrow = c(1,2))
barplot(
  peaks_by_distance,
  las=1,
...
# fisher test having/not having RE
barplot(
  c(
    fisher.numRE.transdev[2, 1] / fisher.numRE.transdev[1, 1],
    fisher.numRE.transdev[2, 2] / fisher.numRE.transdev[1, 2],
...
# types of peaks by distance, all vs diff.reg
par(mfrow = c(1,2))
barplot(
  all_vs_diffpeaks_by_distance,
  las=2,
...
```{r motif_PLOT}
pdf("graphics/all_motifs_by_genomic_feature.pdf", height = 24, width = 6)
ggplot(data = atac_motifs_homercat_minim, aes(x = category, y = motif, # esto ta der carajo
                                              color = `logqval`, size = as.factor(size_pct))) +
  geom_point() +
  scale_color_continuous_sequential(palette = "BluGrn")+

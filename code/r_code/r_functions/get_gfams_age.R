...
parse_geneage_table <- function(dollo_table,ages_table,reverse_order = FALSE){
  #1 Load the gfam age table
  dollo_table <- dollo_table
  colnames(dollo_table)[1] <- "gfam"
  
  if (reverse_order == FALSE){
    age_num <- seq(1:nrow(ages_table))
  } else {
    age_num <- rev(seq(1:nrow(ages_table)))
  ...
  gfams_age <- data.frame(
    ...
    age_num = age_num_asc,
    age = colnames(dollo_table_filt)[age_num_asc + 1] #nice
  )
  
  if (reverse_order == TRUE){
    gfams_age$age_num <- 
      age_lookup[match(age_num_asc,age_lookup$orig),"rev"]
  }
